...
var lmsURI;
browser.storage.sync.get('squeezifyOptions').then((result) => {
		...
		for (var i = 0, l = trackLinks.length; i < l; i++) {
			...
			if (trackLink.clientHeight > 0 && trackLink.clientWidth > 0) {
				try {
					...
					toastNotification("LMS call failed with error <strong>" + error.message + "</strong>, please check server configuration in the addon configuration page", "error");
					return;
				}
			}
		}
